...
const waitForElementImages = (container: HTMLElement): Promise<void> => {
...
export const generateGuidePDF = async ({ title, topic, bookData, previewElement, coverElement }: GeneratePDFOptions) => {
  const doc = new jsPDF({
  ...
  // ==========================================
  // COVER PAGE - Use html2canvas if coverElement provided
  // ==========================================
  
  if (coverElement) {
    // Wait for cover images to load
    await waitForElementImages(coverElement);
    
    // Capture the BookCover component as canvas
    ...
    const maxCoverWidth = pageWidth - (coverMargin * 2);
    const maxCoverHeight = pageHeight - (coverMargin * 2);
    
    // Scale cover to fit page while maintaining aspect ratio
    const coverAspect = coverCanvas.width / coverCanvas.height;
    ...
    doc.addImage(coverImgData, 'PNG', coverX, coverY, coverWidth, coverHeight);
  } else {
    // Fallback: Manual cover drawing if no element provided
